...
export default function Product({ data: { product, allProducts } }) {
  const { checkout, addVariantToCart } = React.useContext(StoreContext)


  const handleCheckout = (variantId, quantity) => {
  ...
  // console.log('hered', allProducts.nodes);
  // console.log('prdId', product.productType);


  const allproductsData = allProducts.nodes
  const collectionId = product.collections


  const producttypeID = product.productType

  // console.log('quantity', quantity)
  // console.log('vart Inve', variant.inventoryQuantity)
            ...
            <div className="row justify-content-between pt-5 mt-5">
              <div className="col-md-5 position-relaative">
                ...
                <div className="d-block d-md-none pb-4">
                  ...
                  {variant.inventoryQuantity <= quantity && variant.availableForSale === true && <p className="mb-0 text-end pt-3 text-danger font_xsm ps-2">Only {variant.inventoryQuantity} quantity available of selected variant</p>}
                </div>
              ...
              <div className="col-md-6">
                ...
                <div>
                  ...
                  {variant.availableForSale != true && <p className="font_xs primary_color">Selected variant is sold out. Please try different combinations. </p>}
                    ...
                    <div className="row">
                      ...
                      <div className="d-block d-md-none pt-4">
                        ...
                        <div className="d-grid mt-3">
                          <button onClick={() => handleCheckout(productVariant.storefrontId, quantity)}
                            disabled={variant.availableForSale === false}
                            className="btn btnDanger px-5 text-white rounded-0 Aftika_ExtraLight text-uppercase">
                            {variant.availableForSale === false ? 'Out of stock' : 'Buy Now'}
                          </button>
                        </div>
                      </div>
                      {/* <div className="d-grid">
                ...
                <div className='d-flex justify-content-between align-items-center'>
                  <div className="d-flex align-items-center">
                    <NumericInput
                      ...
                      max="100"
                    />
                    {variant.inventoryQuantity <= quantity && variant.availableForSale === true && <p className="mb-0 text-center text-danger font_xsm ps-2">Only {variant.inventoryQuantity} quantity available of selected variant</p>}
                  </div>

                  <div>
                    <span>{actualPrice?.map((item) => (
                      <div>
                        {item.compareAtPrice && item.compareAtPrice != 0.00 && <del>£{item.compareAtPrice}</del>}
                        <span className="Aftika_Bold font-2xl">£{item.price}</span>
            ...
            <div className="d-md-none">
              {
                <div className="row pt-md-5 justify-content-center mt-md-5 productSlick px-3 px-md-0">
                  ...
                  <Slider {...settings} className="px-0">
                    {
                      allproductsData?.filter((item) => item.productType == producttypeID).slice(0, 4).map((data) => (
                        <Link to={data.slug}>
                          <img src={data.images[0].gatsbyImageData.images.fallback.src} alt="" />
                          <p className={`mb-1 pt-4 pt-md-5 pb-3  Aftika_ExtraLight link-text text-dark text-center text-md-start ${fontSm}`}>{data.title}</p>
                ...
                <div className="row justify-content-between">
                  {
                    allproductsData?.filter((item) => item.productType == producttypeID).slice(0, 4).map((data) => (
                      <div className="col-md-3">
                        <Link to={data.slug}>
                          <img src={data.images[0].gatsbyImageData.images.fallback.src} alt="" />
                          <p className={`mb-1 pt-4 pt-md-5 pb-1  Aftika_ExtraLight link-text text-dark text-center text-md-start ${fontSm}`}>{data.title}</p>
                          <p className={`Aftika_Bold text-dark text-center text-md-start ${fontSm}`}>£{priceRangeV2.minVariantPrice.amount}</p>
                        </Link>
                      </div>
                    ))
